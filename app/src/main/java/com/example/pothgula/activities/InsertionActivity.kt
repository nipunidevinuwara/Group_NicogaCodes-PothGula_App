...
class InsertionActivity : AppCompatActivity() {
    ...
    private fun saveEmployeeData() {
        ...
        if (empName.isEmpty()) {
            etEmpName.error = "Please enter name of book"
        }
        if (empAge.isEmpty()) {
            etEmpAge.error = "Please enter price"
        }
        if (empSalary.isEmpty()) {
            etEmpSalary.error = "Please enter quantity"
        }
        ...
        dbRef.child(empId).setValue(employee)
            .addOnCompleteListener {
                Toast.makeText(this, "Request inserted successfully", Toast.LENGTH_LONG).show()
